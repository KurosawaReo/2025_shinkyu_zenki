	...
	//背景画像.
	{
		...
		for (int x = 0; x < WINDOW_WID+size.x; x += size.x) {
			for (int y = -size.y; y < WINDOW_HEI; y += size.y) {
				...
				//座標を少しずつずらす.
				INT_XY newPos = {
					_int(x - fmod(p_data->counter, size.x)), //sizeで割った余り.
					_int(y + fmod(p_data->counter, size.y))
				};
				//通常モード.
				{
